...
async fn smoke_test_find_files() {
    ...
    for (pattern, description) in search_patterns {
        ...
        match files_result {
            Ok(files) => {
                println!("✓ File search for {} succeeded ({} files found)", description, files.len());
            ...
            Err(e) => {
                println!("Note: File search for {} failed (may be expected): {}", description, e);
...
async fn smoke_test_find_text() {
    ...
    for (pattern, description) in search_patterns {
        ...
        match matches_result {
            Ok(matches) => {
                println!("✓ Text search for {} succeeded ({} matches found)", description, matches.len());
            ...
            Err(e) => {
                println!("Note: Text search for {} failed (may be expected in empty directory): {}", description, e);
                // Ensure error is properly structured
                assert!(!format!("{}", e).is_empty(), "Error message should not be empty");
            }
        }
...
async fn smoke_test_find_symbols() {
    ...
    for (pattern, description) in search_patterns {
        ...
        match symbols_result {
            Ok(symbols) => {
                println!("✓ Symbol search for {} succeeded ({} symbols found)", description, symbols.len());
            ...
            Err(e) => {
                println!("Note: Symbol search for {} failed (may be expected without LSP): {}", description, e);
                // Ensure error is properly structured
                assert!(!format!("{}", e).is_empty(), "Error message should not be empty");
            }
        }
...
async fn smoke_test_search_error_handling() {
    ...
    for (pattern, description) in invalid_patterns {
        ...
        match file_result {
            Ok(files) => {
                println!("Note: File search with {} unexpectedly succeeded ({} results)", description, files.len());
            }
            Err(e) => {
                println!("✓ File search with {} failed as expected: {}", description, e);
            }
        }
        ...
        match text_result {
            Ok(matches) => {
                println!("Note: Text search with {} unexpectedly succeeded ({} results)", description, matches.len());
            }
            Err(e) => {
                println!("✓ Text search with {} failed as expected: {}", description, e);
            }
        }
        ...
        match symbol_result {
            Ok(symbols) => {
                println!("Note: Symbol search with {} unexpectedly succeeded ({} results)", description, symbols.len());
            }
            Err(e) => {
                println!("✓ Symbol search with {} failed as expected: {}", description, e);
            }
        }
    }
...
async fn smoke_test_concurrent_search_operations() {
    ...
    let tasks = vec![
        ...
        tokio::spawn({
            ...
            async move {
                let client = OpenCodeClient::new(&url);
                ("find_symbols", client.find_symbols("main").await.map(|r| r.len()))
            }
        }),
    ];
    ...
    for task in tasks {
        ...
        match result {
            Ok(count) => {
                println!("✓ Concurrent {} operation completed successfully ({} results)", operation, count);
            }
            Err(e) => {
                println!("Note: Concurrent {} operation failed (may be expected): {}", operation, e);
            }
        }
    }
...
async fn smoke_test_search_consistency() {
    ...
    for i in 0..3 {
        ...
        match files_result {
            Ok(files) => {
                results.push(files);
                println!("✓ Search consistency test {} completed ({} files)", i + 1, results[i].len());
            }
            Err(e) => {
